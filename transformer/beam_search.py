...
class SequenceBeamSearch(object):
    ...
    def __init__(self, encoder_input_tokens, symbols_to_logits_fn, vocab_size, batch_size,
        ...
        for clean in [vocabulary.PAD, '{', '}', '_', '^', '[', ']', '\\limits']:
    ...
    def _continue_search(self, state):
        ...
        alive_seqs = state[_StateKeys.ALIVE_SEQ]
    ...
    def _calculate_translation_token_score(self, alive_seq):
        """
        :param alive_seq: [batch_size, beam_size, CURE_INDEX + 1]
        :return: Returns a score of [batch_size, beam_size]
        """
        # Calculate the same token embeddings
        shape = tf.shape(alive_seq)
        batch_size = shape[0]
        beam_size = shape[1]
        seq_len = shape[2]
...
def sequence_beam_search(
    ...
    Args:
      encoder_input_tokens: Tensor of [batch_size, input_length]
